...
[DIAGRAM: A user with a confused expression types "Tell me about history." A single arrow points to a 'Retrieval System' icon, which points to a single, thick book labeled "Ancient Roman Toilets." Another arrow points to an LLM icon, which is shrugging, holding a speech bubble that says, "Uh... it was a thing?"]
...
[DIAGRAM: A stylized LLM icon wearing a trench coat and a fedora, holding a magnifying glass. It's looking at a whiteboard with a flowchart: "Initial Query -> Plan Search -> Execute Search -> Analyze Results -> *Is more info needed?* (YES -> Refine Query -> Repeat) (NO -> Formulate Answer)".]
...
[DIAGRAM: A cartoon LLM brain with various thought bubbles and attached tools. One thought bubble says "PLAN: Break down query." Another says "REFLECT: Are these results good enough?" An arm reaches out to a toolbox labeled "TOOLS" containing icons for "Search API," "Calculator," "Database Query." A scroll labeled "MEMORY" is next to it.]
...
[DIAGRAM: A stylized LLM brain with a complex thought bubble above it. Inside the thought bubble is a multi-step flowchart:
1.  **User Query:** "Compare electric vs. gasoline car long-term costs and environmental impact."
2.  **Thought:** "This is complex. Need to break it down."
3.  **Sub-Questions:**
    *   "What are the initial costs of EVs?"
    *   "What are the initial costs of gasoline cars?"
    *   "What are the *operational* costs (fuel/electricity, maintenance) for each?"
    *   "What's the manufacturing environmental impact for each?"
    *   "What's the operational environmental impact for each?"
    *   "Are there government incentives for EVs?"
4.  **Plan:** "Retrieve initial costs first, then operational, then environmental. Make sure to get *both* sides for each point."
5.  **Anticipate:** "Might need to look for battery degradation data for EV long-term costs."
This thought bubble is connected by an arrow to a 'Search Tool' icon.]
...
[DIAGRAM: An LLM icon, still in its trench coat, now has a multi-pouched utility belt. Each pouch contains a different icon:
- Pouch 1: Magnifying glass (labeled "Semantic Search")
- Pouch 2: A small book with a keyword highlighted (labeled "Keyword Search")
- Pouch 3: A spiderweb/network diagram (labeled "Graph DB Query")
- Pouch 4: Two arrows pointing at each other (labeled "Re-ranker")
- Pouch 5: A scroll with '...' (labeled "Summarizer")
- Pouch 6: A speech bubble with a question mark (labeled "Query Reformer")
- Pouch 7: A calculator icon (labeled "Calculator API")
- Pouch 8: A globe with a search bar (labeled "Web Search API")]
...
[DIAGRAM: A circular flow diagram labeled "The Reflection-Retrieval Loop."
- Start: "Initial Query"
- Arrow to: "Agent Plans & Executes Search (using tools)"
- Arrow to: "Retrieves Information"
- Arrow to: "Agent Reflects & Evaluates Results" (Thought bubble: "Is this complete? Relevant? Contradictory? What's missing?")
- Branch 1 (Yes, it's good!): Arrow to "Formulate Final Answer"
- Branch 2 (No, needs more!): Arrow back to "Agent Plans & Executes Search (with refined query/strategy)"
This loop emphasizes the continuous nature of the process.]
...
[DIAGRAM: A chain of connected boxes, representing a multi-hop process.
- **Box 1 (Start):** "User Query: What company did the inventor of the light bulb found, and when was it established?"
- **Arrow to Box 2:** "Agent Thought: First, I need to know *who* invented the light bulb."
- **Box 2 (Search 1):** "Query: 'Who invented the light bulb?'"
- **Arrow to Box 3:** "Retrieval Result: Thomas Edison."
- **Arrow to Box 4:** "Agent Thought: Now that I know it's Edison, I need to find his company."
- **Box 4 (Search 2):** "Query: 'Companies founded by Thomas Edison.'"
- **Arrow to Box 5:** "Retrieval Result: Edison Electric Light Company."
- **Arrow to Box 6:** "Agent Thought: Great! Now, when was *that company* established?"
- **Box 6 (Search 3): "Query: 'When was Edison Electric Light Company established?'"
- **Arrow to Box 7 (End):** "Retrieval Result: 1878. Final Answer Formulated."]
...
[DIAGRAM: A stylized LLM brain with a speech bubble labeled "User Query: 'How do I do the thing?'". An arrow points to another speech bubble labeled "Agent's Internal Monologue: 'Hmm, 'the thing' is vague. Based on past conversation, user was asking about 'setting up a Python environment'. I should rephrase this.'". An arrow from the internal monologue points to a final speech bubble labeled "Rewritten Query for Retrieval: 'Best practices for setting up a Python development environment on Windows.'"]
...
[DIAGRAM: A stylized LLM icon (our agent detective) with a thought bubble above its head. Inside the thought bubble, a question mark morphs into a magnifying glass, then splits into two paths:
Path 1: A user icon with a speech bubble asking "What do you mean by 'it'?"
Path 2: A quick, small search icon with a label "Scouting Search: 'related terms for X'".
The agent looks thoughtful, not confused.]
...
[DIAGRAM: An LLM icon (our agent detective) triumphantly holding up a stack of documents, wearing a satisfied expression. Above its head, a lightbulb icon lights up with "AHA!" and a checklist with all items ticked. A thought bubble next to it says: "All sub-questions covered! Information consistent! High confidence! Time to synthesize!"]
...
[DIAGRAM: A traditional RAG system represented as a single conveyor belt spitting out documents, with a clipboard labeled "Precision/Recall" next to it, looking a bit bored. Below it, an Agentic RAG system is a complex Rube Goldberg machine with gears, levers, and interconnected parts. A new, more complex clipboard next to it is labeled "Holistic Evaluation: Process, Adaptiveness, Efficiency, Output Quality." The agent is wiping sweat from its brow.]
...
[DIAGRAM: A flowchart of an agent's steps, with little thought bubbles indicating measurements.
- "Initial Query"
- Arrow to "Thought 1 (Query Reformulation)" -> Bubble: "Query Reformulation Quality Score"
- Arrow to "Search 1 (Tool: Vector DB)" -> Bubble: "Retrieval Step Count: 1"
- Arrow to "Reflection 1" -> Bubble: "Self-Correction Attempt: Yes"
- Arrow to "Search 2 (Tool: Keyword Search)" -> Bubble: "Retrieval Step Count: 2, Tool Switch Efficiency"
- Arrow to "Reflection 2" -> Bubble: "Self-Correction Success: Yes"
- Arrow to "Final Answer" -> Bubble: "Total Latency"
This diagram should have small 'dials' or 'scorecards' next to each measurement.]
...
[DIAGRAM: A user icon with a thought bubble: "Plan my dream vacation to Mars!" An arrow points to an LLM Agent icon, which has many gears turning and tools whirring. Another arrow points to a 'Human Evaluator' icon with a clipboard, checking off items: "Factual Correctness? Y/N", "Completeness? Y/N", "Coherence? Y/N", "User Intent Met? Y/N".]
...
[DIAGRAM: A cartoon LLM icon (our agent detective) looking frustrated, holding a magnifying glass. Its thought bubble shows a tangled, broken flowchart with arrows going in circles. Below it, a human hand is pointing to a section of the tangled flowchart with a red circle, labeled "Aha! Found the planning hallucination!"]
