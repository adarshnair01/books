...
[DIAGRAM: A cartoon detective with a magnifying glass looking intently at a scatter plot. The plot shows temperature on the x-axis and ice cream sales on the y-axis, with individual points scattered. A clear upward-sloping trend line is drawn through the points. The detective has a "Eureka!" thought bubble above their head, perhaps with a smaller icon of an ice cream cone.]
...
[DIAGRAM: Three panels, clearly separated.
Panel 1: **Correlation**. Two distinct circles (A and B) with a significant overlapping section. Caption: "A and B hang out together. A lot. (Like BFFs!)"
Panel 2: **Causation**. Circle A with a bold, thick arrow pointing directly to Circle B. Caption: "A *makes* B happen. No doubt. (Like pressing a button!)"
Panel 3: **Spurious Link**. Two distinct circles (C and D) far apart, with a small, almost invisible, third circle (X) labeled "Hidden Factor?" that has faint, dashed arrows pointing to both C and D. Or, C and D with a big, confused question mark between them and a thought bubble saying "WTF?" above them. Caption: "C and D look related, but it's a trick! (Maybe X is the real puppet master, or it's just random chance!)"]
...
[DIAGRAM: Three small scatter plots side-by-side.
Plot 1: **Positive Relationship**. Dots tightly clustered along an upward sloping invisible line (bottom-left to top-right). Label: "Strong Positive (More Treats, More Purrs!)"
Plot 2: **Negative Relationship**. Dots tightly clustered along a downward sloping invisible line (top-left to bottom-right). Label: "Strong Negative (More TV, Less Nap!)"
Plot 3: **No Relationship**. Dots randomly scattered across the entire plot area, forming a loose, circular blob. Label: "No Relationship (Trips vs. Purrs, huh?)"
Each plot has faint X and Y axes.]
...
[DIAGRAM: A scatter plot with multiple data points. A single, clear straight line (the regression line) is drawn through the center of the points. From several individual data points, short, dashed vertical lines extend down (or up) to the regression line, illustrating the "residuals" or "errors". A small thought bubble above the regression line says: "My job is to make these dashed lines as short as possible (on average)!"]
...
[DIAGRAM: A scatter plot with numerous data points. Several possible straight lines are faintly drawn in the background, but one prominent, bold straight line (the 'Line of Best Fit') stands out. From three or four data points, dashed vertical lines extend to the bold line, representing the residuals. Next to each dashed line, a small square is drawn, visually representing the 'squared error' for that point. A thought bubble near the bold line says: "I made the sum of all these squares as small as possible!"]
...
[DIAGRAM: A clear scatter plot with a regression line drawn through it. The Y-axis is labeled "Predicted Y" and the X-axis is labeled "X".
1.  A dashed line extends from the origin (X=0) up to where the regression line intersects the Y-axis. This intersection point is clearly labeled "$b_0$ (Intercept)". A small thought bubble near $b_0$ says: "What y is when x is ZERO!"
2.  Along the regression line, a small right-angle triangle is drawn, illustrating "rise over run". The horizontal segment is labeled "1 unit change in X". The vertical segment is labeled "$b_1$ (Slope)". A thought bubble near $b_1$ says: "How much Y changes for every 1 unit of X!"]
...
[DIAGRAM: A scatter plot with multiple data points and a clear regression line. Several data points are highlighted, and a dashed vertical line extends from each highlighted point down (or up) to the regression line. Each dashed line is labeled "Residual". One point far above the line has a long positive residual, one far below has a long negative residual. A thought bubble says: "These are the stories my line *missed*!"]
...
[DIAGRAM: A 3D scatter plot. The X-axis is "Square Footage", the Y-axis is "Number of Bathrooms", and the Z-axis (coming out of the page) is "House Price". Data points are scattered in this 3D space. A flat, transparent "plane" (like a sheet of glass) is gently angled and fitted through the center of these 3D points, showing the "line of best fit" but in 3D. A label points to the plane: "The 'Plane of Best Fit' - our story in 3D!"]
...
[DIAGRAM: A visual representation of "ceteris paribus". Imagine a multi-lever control panel. One lever is labeled "Square Footage", another "Bathrooms", another "Distance to City". All levers except "Square Footage" are locked in place. The "Square Footage" lever is being pushed up slightly. An arrow points to an output meter labeled "House Price" which shows a small increase. A thought bubble above the locked levers says: "Everyone else stays put!"]
...
[DIAGRAM: A pie chart divided into two sections.
Larger section (e.g., 75% of the pie) is colored brightly and labeled "Explained Variation by Model (R-squared)".
Smaller section (e.g., 25% of the pie) is colored in a duller shade and labeled "Unexplained Variation (Residuals)".
A cheerful AI agent character is pointing to the "Explained Variation" slice with a proud grin.]
...
Panel 1: A confused business person looking at a screen full of complex regression output (equations, p-values, R-squared numbers). A thought bubble above their head says: "???".
Panel 2: The same business person now looking at a clean, simple infographic or bullet points. An AI agent character is explaining clearly with a pointer. A thought bubble above the business person says: "Aha! So we should invest more in X!"]
...
[DIAGRAM: A scatter plot showing data points forming a clear U-shape or inverted U-shape curve. A straight regression line is drawn through the middle of the curve, clearly missing the pattern of the points. A thought bubble above the line says: "I tried my best, but this data just won't stay straight!"]
...
[DIAGRAM: Two small histograms side-by-side.
Histogram 1: Looks like a perfect bell curve. Label: "Normal Residuals (Good!)"
Histogram 2: Is skewed to one side or has two distinct peaks. Label: "Non-Normal Residuals (Uh oh!)"]
...
[DIAGRAM: A 4-panel comic strip, each panel showing a different broken assumption with its visual red flag:
Panel 1: Scatter plot with a clear curve, straight line through it. Caption: "Non-Linearity: My line's missing the plot!"
Panel 2: Residual plot showing waves/streaks (e.g., for time series). Caption: "Autocorrelation: My errors are gossiping!"
Panel 3: Residual plot showing a cone/fan shape. Caption: "Heteroscedasticity: My crystal ball's blurry sometimes!"
Panel 4: Skewed histogram of residuals. Caption: "Non-Normality: My leftovers are weirdly distributed!"]
...
[DIAGRAM: A scatter plot with existing data points and a clear regression line. The line extends beyond the existing data to the right. A new, distinct data point (a star or a larger dot) is placed further along the X-axis, outside the original data range. A dashed vertical line goes from this new X-value up to the extended regression line, and then a dashed horizontal line goes from the regression line to the Y-axis, clearly indicating the "Predicted Y for New X". A thought bubble above the predicted point says: "The future, according to my data!"]
...
[DIAGRAM: A simple scatter plot. X-axis: "Distance (miles)", Y-axis: "Delivery Time (minutes". Dots generally trend upwards, but with some spread. A faint, imaginary upward-sloping line is visible. A thought bubble says: "Looks like longer distances mean longer waits!"]
